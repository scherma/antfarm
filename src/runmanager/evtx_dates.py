...
def parsed_date(dstr):
	...
	ts = arrow.get(dstr)
	#try:
	#	ts = arrow.get(datetime.strptime(dstr, '%Y-%m-%d %H:%M:%S'), tz.gettz("UTC"))
	#except ValueError:
	#	ts = arrow.get(datetime.strptime(dstr, '%Y-%m-%d %H:%M:%S.%f'), tz.gettz("UTC"))
	return ts
