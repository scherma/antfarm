...
def main():
    ...
    if len(unregistered) > 0:
        ...
        while not isinstance(selected, libvirt.virDomain):
            try:
                selected = unregistered[int(input("Please choose a VM no.: "))]
        ...
        input("Press enter to confirm this selection (ctrl + c to abort)")
        
        cursor.execute("""INSERT INTO "victims" (libvirtname, uuid, diskfile, status) """ +
                       """VALUES (%s, %s, %s, %s)""",
                       (selected.name(), selected.UUIDString(), chosendisk, 'pre-prod'))
